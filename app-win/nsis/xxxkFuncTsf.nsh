...
!macro xxxkMacroTsfToFolder arg0 # arg0 是文件夹路径
    ...
    # 可以随意使用变量了（但不能使用形参）
    # 释放 TSF 模块：app-win\yong\tsf → 指定文件夹\tsf
    SetOutPath "$R0"
    File "${XXXK_DIR_APP_WIN}\yong\tsf\tsf-reg.exe"
    File "${XXXK_DIR_APP_WIN}\yong\tsf\tsf-reg64.exe"
    File "${XXXK_DIR_APP_WIN}\yong\tsf\install.bat"
    File "${XXXK_DIR_APP_WIN}\yong\tsf\uninstall.bat"
    ...
    ${EndIf}
    File "${XXXK_DIR_APP_WIN}\yong\tsf\yong.dll"
    File "${XXXK_DIR_APP_WIN}\yong\tsf\yong64.dll"
    Delete "$R0\*.del"
